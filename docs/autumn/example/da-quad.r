...
table(true=Xnew$cat, pred=myprdq$class)   # 真のクラスと予測されたクラスの比較
## 予測を真の分類と比較
myprdq$class # 予測
Xnew$cat     # 真の値
## 結果の図示
...
table(true=Xnew$cat, pred=myprdl$class)   # 真のクラスと予測されたクラスの比較
## 予測を真の分類と比較
myprdl$class # 予測
Xnew$cat     # 真の値
...
## install.packages("ISLR") # パッケージのインストール(必要な場合)
require(ISLR)
head(Smarket)
pairs(subset(Smarket, select=paste("Lag", 1:5, sep="")),
      col=rainbow(2)[Smarket$Direction]) 
Smarket.train <- subset(Smarket, Year < 2005) # 訓練データ(2004年まで)
Smarket.test <- subset(Smarket, Year == 2005) # 試験データ(2005年)
(myld <- lda(Direction ~ Lag1 + Lag2, data=Smarket.train))
prdl.train <- predict(myld) # 訓練データに対する予測結果
table(true=Smarket.train$Direction, pred=prdl.train$class)
(myqd <- qda(Direction ~ Lag1 + Lag2, data=Smarket.train))
prdq.train <- predict(myqd) # 訓練データに対する予測結果
table(true=Smarket.train$Direction, pred=prdq.train$class)
prdl.test <- predict(myld, newdata=Smarket.test) # 試験データに対する予測結果
table(true=Smarket.test$Direction, pred=prdl.test$class)
prdq.test <- predict(myqd, newdata=Smarket.test) # 試験データに対する予測結果
table(true=Smarket.test$Direction, pred=prdq.test$class)
