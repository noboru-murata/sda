...
** 検定の準備
   - *帰無分布*: 帰無仮説が正しい場合に検定統計量が従う分布
     (統計量が取り得る値の範囲を予想する)
     - 帰無仮説を *棄却*: 帰無仮説は誤っていると判断すること
     - 帰無仮説を *受容*: 帰無仮説を積極的に棄却できないこと
   - *棄却域*: 帰無仮説を棄却するために決める領域
    (帰無仮説の下で統計量の取り得る値の範囲外の領域)
   - 検定の誤り
...
** 検定の設計指針
   ...
   - *検出力*: 第二種過誤が *起きない確率*
     ...
     \end{equation}
   - *有意水準*: 第一種過誤が起きる確率(サイズ)として許容する上限

** 棄却域の決め方
   - 検定を構成する場合の一般的な戦略: \\
     棄却域 $R_{\alpha}$ は
     - *サイズを小さく* (有意水準 $\alpha$ 以下に) 抑えた上で
     - 可能な限り *検出力を大きく* する
     ように設定する
     \begin{align}
